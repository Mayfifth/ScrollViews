...
class PagedScrollViewController: UIViewController, UIScrollViewDelegate {
    ...
    //UIImageView 배열로써 optional임, 왜냐하면 필요할 때 로딩하므로 !
    var pageViews: [UIImageView?] = []
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
        // 1. 5개 photo로부터 UIImage 배열 생성
        ...
        //    나중에 로딩할 때 실제 pageImages 에서 가져옴
    ...
    func loadPage(page: Int) {
        ...
        if let pageView = pageViews[page] {
            ...
            // 2. pageView 가 nil이면 로딩해야 함
            //    y 오프셋은 0 이고  x 오프셋은 화면너비*페이지인덱스로 설정
            var frame = scrollView.bounds
            frame.origin.x = frame.size.width * CGFloat(page)
            frame.origin.y = 0.0
            
            // 3. 새로운 UIImageView 생성하고 scrollView에 추가
            let newPageView = UIImageView(image: pageImages[page])
            newPageView.contentMode = .ScaleAspectFit
            newPageView.frame = frame
            scrollView.addSubview(newPageView)
            
            // 4. 새로 생성한 newPageView 를 배열의 nil 대신 삽입
    ...
    func purgePage(page: Int) {
        ...
        // pageViews[page]가 nil이 아니면 scrollView 에서 제거하고 
